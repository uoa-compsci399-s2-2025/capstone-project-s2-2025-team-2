...
export default class GoogleOAuthService {
   ...
   */
  private async saveUserToFirestore(uid: string, email: string, displayName?: string): Promise<void> {
    try {
      const userData = {
        email: email,
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          if (userDoc.exists) {
            ...
            console.log(`Updated existing Firestore document for UID: ${userRecord.uid}`)
          } else {
            // Document doesn't exist, create it (this should not happen for existing users)
            console.warn(`Firestore document missing for existing user UID: ${userRecord.uid}, creating it now`)
            await userDocRef.set({
              email: googleUserInfo.email,
              displayName: googleUserInfo.name,
              createdAt: this.getNewZealandTime()
            })
            console.log(`Created missing Firestore document for existing user UID: ${userRecord.uid}`)
          }
        } catch (firestoreError) {
          console.error('Failed to handle Firestore document for existing user:', firestoreError)
      ...
      if (isNewUser) {
        try {
          ...
          console.log(`Successfully created Firestore document for new user UID: ${userRecord.uid}`)
        } catch (firestoreError) {
   ...
   */
  public async verifyTokenAndAuthenticate(idToken: string): Promise<GoogleOAuthResponse> {
    try {
