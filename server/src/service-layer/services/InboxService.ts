...
import { ChatRoomResponse, ConversationListResponse } from "../dtos/response/ChatRoomResponse";
...
export class InboxService {
  ...
  async getConversations(userId: string): Promise<ConversationListResponse> {
    ...
    for (const chatRoom of chatRooms) {
      const messages = await this.inboxRepository.getMessagesByChatRoomWithLimit(
        chatRoom.id!,
        50
      );
  ...
  async getChatRoomById(chatRoomId: string, userId: string): Promise<ChatRoomResponse | null> {
    ...
    const messages = await this.inboxRepository.getMessagesByChatRoom(chatRoomId);
