...
export class ReagentService {
  ...
  async getReagentsByCategory(categories:ReagentCategory[]): Promise<Reagent[]> {
     const reagentsSnapshot = await FirestoreCollections.reagents.where('categories', 'array-contains-any', categories).get()
     return reagentsSnapshot.docs.map((doc) => doc.data())
  }
}
